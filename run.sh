...
python3 clicc.py examples/rope.clic        -o build/rope.metal
python3 clicc.py examples/swiglu.clic      -o build/swiglu.metal
python3 clicc.py examples/gqa.clic         -o build/gqa.metal
...
for r in saxpy gemm gemm_tiled gemm_i8 bmm mathfns consts u64mix keccak sha3_256 keccak256 linear_relu linear linear_gelu reduce softmax layernorm rmsnorm attention flash_attn rope swiglu gqa conv2d conv2d_mc maxpool transpose scan argmax vec4 clamp scale_half quant silu collatz sha256 raster; do
    echo
